...
const SearchModal = ({ searchModal, setSearchModal }) => {
      ...
      if (e.key === "Enter") {
        ...
        if (e.target.value != "") {
          ...
          token = await axios.get(
            `/coinranking/v2/coins?offset=0&orderBy=marketCap&limit=50&orderDirection=desc&referenceCurrencyUuid=yhjMzLPhuIDl&timePeriod=24h&search=${e.target.value}&tags[]=brc-20`
          );
        } else
          token = await axios.get(
            `/coinranking/v2/coins?offset=0&orderBy=marketCap&limit=50&orderDirection=desc&referenceCurrencyUuid=yhjMzLPhuIDl&timePeriod=24h&search=&tags[]=brc-20`
          );
        dispatch({ type: "GET_UNISAT_DATA", payload: token.data.data });
        dispatch({ type: "SET_LOADING", payload: false });
      }
  ...
  return (
    <div
      className={`search-modal ${searchModal ? "open dark:bg-[#19122b]" : ""}`}
    >
      <button
        onClick={() => handleClose()}
